...
isSafeModule m = do
  ...
  where
    ...
    packageTrusted dflags md
        | thisPackage dflags == moduleUnitKey md = True
        | otherwise = trusted $ getPackageDetails dflags (moduleUnitKey md)
...
isHomeModule m = GHC.moduleUnitKey m == mainUnitKey
...
wantInterpretedModuleName modname = do
   ...
   dflags <- getDynFlags
   when (GHC.moduleUnitKey modl /= thisPackage dflags) $
