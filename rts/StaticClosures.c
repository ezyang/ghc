...
checkStaticClosures(W_ size_b, StgClosure **section_start) {
    ...
    for (
        pp = section_start;
        pp < section_end;
        pp += sizeofW(StgStaticClosureDesc) + payload_size
    ) {
        ...
        case CONSTR_NOCAF_STATIC:
          {
            ...
            payload_size = info->layout.payload.ptrs +
...
initStaticClosures(void)
{
    if (global_block == NULL) {
        ...
        stg_CHARLIKE_static_closure_ind =
            (StgIntCharlikeClosure*)global_block->free;
...
allocateStaticClosures(StaticClosureInds *base_sci, bdescr **current_block_p)
{
    ...
    for (sci = base_sci; sci != NULL; sci = sci->link) {
        StgClosure **pp; // for easier pointer arithmetic:
                         // technically it's StgStaticClosureDesc*
        W_ payload_size;
        for (
            pp = sci->start;
            pp < sci->end;
            ...
            const StgInfoTable *info_ptr =
                (StgInfoTable*)UNTAG_CLOSURE((StgClosure*)p->t.tagged_info);
            ...
            current_block->free += size_w;
            ASSERT((StgClosure*)&q->payload[i] ==
                   (StgClosure*)current_block->free);
