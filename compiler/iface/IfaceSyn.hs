    ...
    = ASSERT ( ifName d1 == ifName d2 )
      do check (ifRoles d1 == ifRoles d2) roles_msg
         ...
         cons <- case (ifCons d1, ifCons d2) of
                    ...
                    (r@IfDataFamTyCon, IfDataFamTyCon) -> return r
                    (r@(IfDataTyCon cs1), IfDataTyCon cs2) -> do
                        zipWithM_ (checkEqCon env) cs1 cs2 >> return r
                    (r@(IfNewTyCon c1), IfNewTyCon c2) ->
                        checkEqCon env c1 c2               >> return r
                    _ -> failME
...
mergeIfaceDecl d1@IfaceSynonym{} d2@IfaceSynonym{}
    ...
    = ASSERT( ifName d1 == ifName d2 )
      do check (ifRoles d1 == ifRoles d2) roles_msg
         check (eqIfaceType env (ifSynRhs d1) (ifSynRhs d2)) empty
         return d1

mergeIfaceDecl d1@IfaceFamily{} d2@IfaceFamily{}
    | Just env <- eqIfaceTvBndrs emptyIfRnEnv2 (ifTyVars d1) (ifTyVars d2)
    = ASSERT ( ifName d1 == ifName d2 )
      do check (eqIfaceType env (ifFamKind d1) (ifFamKind d2)) empty
         fam_flav <- case (ifFamFlav d1, ifFamFlav d2) of
             ...
             (r@(IfaceClosedSynFamilyTyCon ax1),
                ...
                check (eqClosedFamilyAx ax1 ax2) empty
                                                    >> return r
    ...
    = ASSERT( ifName d1 == ifName d2 )
      do check (ifRoles d1 == ifRoles d2) roles_msg
         ...
         let isAbstract d = null (ifCtxt d) && null (ifATs d) && null (ifSigs d)
             ...
             eqATDef _ _ = False
             checkSig (IfaceClassOp n1 def1 ty1) (IfaceClassOp n2 def2 ty2) = do
                check (n1 == n2)
...
mergeIfaceDecl _ _ = failME empty
...
mergeIfaceIdInfo :: IfaceIdInfo -> IfaceIdInfo -> IfaceIdInfo
mergeIfaceIdInfo NoInfo i2 = i2
mergeIfaceIdInfo i1 NoInfo = i1
-- Would like to test that i1 == i2, but no convenient Eq available.
mergeIfaceIdInfo i1 _i2 = i1
