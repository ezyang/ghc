...
loadSrcInterface_maybe doc mod want_boot maybe_pkg
  ...
  = do { hsc_env <- getTopEnv
       ...
       ; case lookupUFM (tcg_ifaces tcg_env) mod of
            Just [] -> panic "Empty tcg_ifaces"
            Just ifaces@(iface:rest) ->
                ASSERT2( all ((== mi_semantic_module iface) . mi_semantic_module) rest,
                         ppr (map mi_top_module ifaces) )
                if tcg_shaping tcg_env
                    then return (Succeeded ifaces)
                    else do -- Make sure the TypeEnvs are loaded

                            r <- initIfaceTcRn $ loadInterface doc (mi_semantic_module iface)
