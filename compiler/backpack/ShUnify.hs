...
computeHoleSubst :: DynFlags -> UnitKey -> ShProvides -> ShRequires -> IO ShHoleSubst
computeHoleSubst dflags pk provs reqs = do
    insts <- unitKeyInsts dflags pk
    let instmap = Map.fromList insts
        proto_hsubst =
            foldl' (\m modname ->
            -- TODO Does order matter? Shouldn't...
                case Map.lookup modname provs of
                    Just (mod, _) -> addToUFM m modname mod
                    Nothing -> case Map.lookup modname instmap of
...
renameHoleModule' dflags env m
  | not (isHoleModule m) = do
        (pk, in_scope) <- renameHoleUnitKey' dflags env (moduleUnitKey m)
...
-- | Substitutes holes in a 'UnitKey', suitable for renaming when
-- an include occurs.
--
-- @p(A -> HOLE:A)@ maps to @p(A -> HOLE:B)@ with @A -> HOLE:B@.
renameHoleUnitKey :: DynFlags -> ShHoleSubst -> UnitKey -> IO UnitKey
renameHoleUnitKey dflags env pk = fmap fst (renameHoleUnitKey' dflags env pk)

-- | Like 'renameHoleUnitKey', but returns the 'ShFreeHoles' of
-- the resulting 'UnitKey'.
renameHoleUnitKey' :: DynFlags
                      -> ShHoleSubst
                      -> UnitKey
                      -> IO (UnitKey, ShFreeHoles)
renameHoleUnitKey' dflags env pk = do
    ShUnitKey { shUnitKeyUnitId = uid
                 , shUnitKeyInsts = insts
                 , shUnitKeyFreeHoles = in_scope } <- lookupUnitKey dflags pk
    if isNullUFM (intersectUFM_C const in_scope env)
        then return (pk, in_scope)
                ...
                newUnitKeyWithScope dflags uid insts'
...
-- 'nameModule' of keys of this map have 'moduleUnitKey' @HOLE@
...
computeShapeSubst hsc_env sh = do
    ...
    fmap mkNameEnv $ mapM (\(modname, n) ->
        do n0 <- initIfaceCheck hsc_env $
                   newGlobalBinder (mkModule holeUnitKey modname)
...
rnModIface :: HscEnv -> UnitKey -> ModIface -> IO ModIface
rnModIface hsc_env pk iface = do
    MASSERT( pk /= holeUnitKey )
    let dflags = hsc_dflags hsc_env
    insts <- unitKeyInsts dflags pk
