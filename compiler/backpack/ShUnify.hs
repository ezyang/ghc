...
-- | Calculate the NON-IDEMPOTENT substitution for filling some requirements
...
renameHoleModule dflags hsubst m = fmap fst (renameHoleModule' dflags hsubst m)

-- | Substitutes holes in a 'Module', like 'renameHoleModule' but also
-- returns 'ShFreeHoles' post-substitution.
...
renameHolePackageKey :: DynFlags -> ShHoleSubst -> PackageKey -> IO (PackageKey, ShFreeHoles)
...
computeShapeSubst :: HscEnv -> AvailShape -> IO ShNameSubst
computeShapeSubst hsc_env sh = do
    let subst = do (modname, as) <- Map.toList (sh_avail_requires sh)
                   a <- as
