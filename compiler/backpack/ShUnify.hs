...
rnModIface hsc_env pk iface = do
    initRnIface hsc_env pk $ do
        topmod <- rnTopModule (mi_top_module iface)
...
rnTopModule :: Rename TopModule
rnTopModule top_module = do
    hmap <- getLclEnv
    dflags <- getDynFlags
    liftIO $ renameHoleTopModule dflags hmap top_module
        ...
        _ | m == m' -> return n
          | isHoleModule m && not (isHoleModule m') -> do
            ...
            case lookupExport (nameOccName n) exports of
                ...
                [] -> do
                    -- This is tricky!  We're in a case where we need to rename
                    -- a Foo{v} from:
                    --
                    --      module A where
                    --          data Foo = Foo
                    --
                    -- Given this definition, Foo{v} IS NOT in the export list.
                    -- But Foo{tc} must be, so all we need to do is find out
                    -- what the *parent* of Foo{v} is. This is recorded in the
                    -- merged hole interface of the package where the interface
                    -- we are renaming came from.
                    uid <- getGblEnv
                    -- TODO: the renaming here is NOT NECESSARY!
                    (hole_iface, _) <- liftIO . initIfaceCheck hsc_env
                                              . withException
                                              $ computeInterface (text "rnIfaceGlobal2") False
                                                    (mkModule uid (moduleName m))

                    -- Grovel in mi_parents for a matching OccName
                    let ps = do (occ, target_occ) <- mi_parents hole_iface
                                guard (occ == nameOccName n)
                                return target_occ
                    case ps of
                        (p:ps) | (b:bs) <- lookupExport p exports
                          -> do MASSERT ( all (p==) ps )
                                MASSERT ( all (b==) bs )
                                liftIO $ setNameModule hsc_env (Just (nameModule b)) n
                        _ -> pprPanic "rnIfaceGlobal" $
