...
-- | Given a 'PackageName', a 'VersionHash', and sorted mapping of holes to
...
-- | Given a 'PackageName' and sorted mapping of holes to
    ...
    case (lookupUFM pk_cache pk, lookupPackage dflags pk) of
        ...
        (Nothing, Just pkg) -> do
            ...
            ASSERT2( pk == new_pk, ppr new_pk $$ ppr pk )
                return (expectJust "lookupPackageKey" (lookupUFM pk_cache pk))
        -- Give up and return a wired package key
        -- TODO: Maybe give a little warning when this happens
        _ -> return (ShWiredPackageKey pk)
...
pprPackageKey pk = getPprStyle $ \sty ->
    ...
    in case shpk of
        ...
        ShWiredPackageKey pk -> ftext (packageKeyFS pk)
...
mkPackageKey :: PackageName
             ...
             -> PackageKey
mkPackageKey (PackageName fsPackageName)
             (VersionHash fsVersionHash) unsorted_holes =
    -- NB: don't use concatFS here, it's not much of an improvement
    fingerprintPackageKey . fingerprintString $
        unpackFS fsPackageName ++ "\n" ++
        unpackFS fsVersionHash ++ "\n" ++
