            ...
            initBkpM src_filename comps $
                forM_ (zip [1..] comps) $ \(i, (cid, pkg)) -> do
                    ...
                    innerBkpM $ do
                        ...
                        -- compile
                        -- (NB: with fat interfaces you always
                        -- have to typecheck first)
...
-- (Boolean indicates whether or not to use the fat interface
-- codepath or not)
data SessionType = ExeSession | TcSession Bool | CompSession Bool
    deriving (Eq)

-- Not toggleable because if all goes according to plan we will
-- kill the fat interface codepath
enableFatInterfaces :: Bool
enableFatInterfaces = True
...
withBkpSession :: UnitId        -- unit ID that we are going to tc/compile
               ...
               -> [FilePath]    -- import directories (for fat interfaces)
               -> BkpM a        -- actual action to run
               -> BkpM a
withBkpSession uid mod_map session_type import_dirs do_this = do
    dflags <- getDynFlags
    ...
    let uid_str = unpackFS (unitIdFS uid)
        ...
        outdir p | CompSession _ <- session_type = sub_comp (key_base p) </> uid_str
    ...
    withTempSession (overHscDynFlags (\dflags ->
      ...
      (case session_type of
        -- Fat interfaces are a bit touchy!
        TcSession True | hscTarget dflags /= HscNothing || gopt Opt_WriteInterface dflags
                       -> flip gopt_set Opt_WriteFatInterface .
                          flip gopt_unset Opt_WriteInterface
                       | otherwise -> id
        CompSession True -> flip gopt_set Opt_FromFatInterface
        -- Without fat interfaces the situation is more banal; just
        -- be sure to write interfaces when we are type-checking
        -- indefinite packages.
        TcSession False | hscTarget dflags /= HscNothing
                        -> flip gopt_set Opt_WriteInterface
                        | otherwise -> id
        CompSession False -> id
        ExeSession -> id) $
      dflags {
        hscTarget   = case session_type of
                        TcSession _ -> HscNothing
                        _ -> hscTarget dflags,
        thisPackage = uid,
        ...
        -- sigOf is used to trigger some alternate codepaths, so it's
        -- important to have this be something accurate.
        sigOf       = Map.fromList insts,
        -- Extra import_dirs let us find fat interface files; i.e. find
        -- where our source lives.
        importPaths  = import_dirs ++ importPaths dflags,
...
withBkpExeSession include_graph do_this = do
    ...
    withBkpSession mainUnitId mod_map ExeSession [] do_this
...
withBkpTcSession pk include_graph do_this = do
    old_eps <- getEpsGhc
    let mod_map = Map.unions (map is_inst_provides include_graph)
    r <- withBkpSession pk mod_map (TcSession enableFatInterfaces) [] do_this
    ...
    updateEpsGhc_ (\eps -> old_eps {
            -- TODO: saving the EST seems to not help
            eps_EST = eps_EST eps
        })
    return r

packageProvides :: PackageConfig -> [Module]
packageProvides cfg = map snd (exposedModules cfg)

withBkpCompSession :: UnitId -> Map ModuleName Module -> LHsUnit -> BkpM a -> BkpM a
withBkpCompSession uid mod_map lunit do_this =
    withBkpSession uid mod_map (CompSession False) [] {- TODO FIXME -} do_this
    -- No setTargets nuttery

-- | Create a temporary session for compilation.
withBkpFatCompSession :: UnitId -> PackageConfig -> BkpM a -> BkpM a
withBkpFatCompSession pk cfg do_this =
    withBkpSession pk Map.empty (CompSession True) (importDirs cfg) $ do
        dflags <- getDynFlags
        -- this is INCREDIBLY GOOFY
        let provs0 = packageProvides cfg
        insts <- liftIO $ unitIdInsts dflags pk
        hsubst <- liftIO $ mkShHoleSubst dflags (listToUFM insts)
        provs <- liftIO $ mapM (renameHoleModule dflags hsubst) provs0
        let our_provs = map moduleName . filter ((==thisPackage dflags) . moduleUnitId) $ provs
        setTargets (map (\modname -> Target (TargetModule modname) True Nothing) our_provs)
        do_this
...
compileUnit uid = do
    ...
    (exposed, hi_dir, obj_files) <- withBkpCompSession uid mod_map lunit $ do
...
-- (Never called on executables.)
--
-- This codepath assumes that fat interface files for the unit in question
-- were installed to the package database.
compileFatUnit :: UnitId -> BkpM ()
compileFatUnit pk = do
    hsc_env <- getSession
    let dflags = hsc_dflags hsc_env
    cid <- liftIO $ unitIdComponentId dflags pk
    let idef_cfg = case lookupComponentId dflags cid of
                    Just idef_cfg -> idef_cfg
                    _ -> pprPanic "at the disco idef_cfg" (ppr cid)
    let comp_name = componentName idef_cfg
    msgUnitId pk
    -- Don't have to do the full include graph kaboodle because it's
    -- pre-calculated for us
    let raw_deps = instantiatedDepends idef_cfg
    insts <- liftIO $ unitIdInsts dflags pk
    cid <- liftIO $ unitIdComponentId dflags pk
    hsubst <- liftIO $ mkShHoleSubst dflags (listToUFM insts)
    deps <- liftIO $ mapM (renameHoleUnitId dflags hsubst) raw_deps
    ...
    forM_ (zip [1..] deps) $
        compileInclude (length deps)
    (exposed, hi_dir, obj_files) <- withBkpFatCompSession pk idef_cfg $ do
        dflags <- getDynFlags
        -- mod_graph <- runShM $ shModGraph pk include_graph pkg
        msg <- mkBackpackMsg
