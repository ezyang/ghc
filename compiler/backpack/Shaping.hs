...
initShM :: HscEnv -> FilePath -> UnitName -> ShM a -> IO (Messages, Maybe a)
...
addShapeToEnv :: UnitName -> (Shape, PackageKey) -> ShM ()
addShapeToEnv name (sh, pk) =
    updateEps_ (\eps -> eps { eps_EST = Map.insert name (sh, pk) (eps_EST eps) })

lookupPackageShape :: UnitName -> ShM (Maybe (Shape, PackageKey))
lookupPackageShape unit_name = do
    eps <- getEps
    dflags <- getDynFlags
    case Map.lookup unit_name (eps_EST eps) of
        Just sh_pk -> return (Just sh_pk)
        Nothing -> case lookupUnitName dflags unit_name of
                    Just pkg -> fmap Just (convertPackageConfig pkg)
                    Nothing -> return Nothing

convertPackageConfig :: PackageConfig -> ShM (Shape, PackageKey)
convertPackageConfig pkg = do
    hsc_env <- getTopEnv
    let pk = packageConfigId pkg
        reqs = Map.empty
    -- TODO: reexport support
    provs <- fmap Map.fromList . forM (exposedModules pkg) $
                \(ExposedModule m exposedReexport _sig) ->
                case exposedReexport of
                    Nothing -> do
                        -- TODO: don't TYPECHECK the entire interface. Just
                        -- read out the mi_exports
                        let mod = mkModule pk m
                        iface <- liftIO $ initIfaceCheck hsc_env (loadSysInterface (text "pkgconfig") mod)
                        return (m, (mod, mi_exports iface))
                    Just _ -> panic "reexports not supported"
...
mergeShapes :: ([LShUnitDecl], Shape)
            -> (LShUnitDecl, Shape)
            -> ShM ([LShUnitDecl], Shape)
mergeShapes (decls, sh1) (decl, sh2) = do
    hsc_env <- getTopEnv
    dflags <- getDynFlags
    ...
    provs <- T.mapM (\(m,  as) -> do m' <- liftIO $ renameHoleModule dflags hsubst m
                                     as' <- mapM doSubst as
                                     return (m', as')) provs0
    let doReqSubst (ms, as) = do ms' <- liftIO $ mapM (renameHoleModule dflags hsubst) ms
                                 as' <- mapM doSubst as
                                 return (ms', as')
    ...
    -- Step 5: Substitute the decls
    let doDeclSubst (L loc (ShDecl summary)) = do
            m' <- liftIO $ renameHoleModule dflags hsubst (ms_mod summary)
            return (L loc (ShDecl summary { ms_mod = m' }))
        doDeclSubst (L loc (ShInclude pk provs reqs)) = do
            pk' <- liftIO $ renameHolePackageKey dflags hsubst pk
            provs' <- T.mapM (liftIO . renameHoleModule dflags hsubst) provs
            reqs' <- T.mapM (liftIO . mapM (renameHoleModule dflags hsubst)) reqs
            return (L loc (ShInclude pk' provs' reqs'))
    decls' <- mapM doDeclSubst (decl:decls)
    return (decls', Shape {
                sh_provides = provs,
                sh_requires = reqs
            })
...
preshape' psh (IncludeD (IncludeDecl{
    ...
    case mb_shape of
        ...
        Just (sh', _) -> fmap (mergePreShapes psh) (renamePreShape ispec (shapeToPreShape sh'))
...
shComputePackageKey :: Bool -> LHsUnit -> ShM PackageKey
shComputePackageKey
    is_exe
...
-- | Shape a 'HsUnit'.
shPackage :: PackageKey -> LHsUnit -> ShM ([LShUnitDecl], Shape)
shPackage pk
    (L loc HsUnit { hsunitName = L _ name
                  , hsunitExports = Nothing -- XXX incomplete
                  , hsunitBody = decls })
...
shPkgDecl :: ([LShUnitDecl], Shape) -> LHsUnitDecl -> ShM ([LShUnitDecl], Shape)
shPkgDecl (decls, sh) (L loc (DeclD dt lmodname@(L _ modname) mb_hsmod)) =
    ...
    summary <- summariseDecl dflags hsc_src outer_m lmodname mb_hsmod
    hsc_env <- getTopEnv
    ...
    avails <- liftTcToSh hsc_src (TopModule { topModSemantic = inner_m
        ...
        updGblEnv (\tcg_env -> tcg_env { tcg_ifaces = mkShIfaces sh
                                       , tcg_shaping = True } ) $
        shModule hsmod
    mergeShapes (decls, sh)
                (L loc (ShDecl summary),
...
shPkgDecl (decls, sh) (L loc (IncludeD IncludeDecl{
    ...
    case mb_shape of
        ...
        Just (sh', pk') -> do
            hsc_env <- getTopEnv
            (incl_sh, incl_pk) <- liftIO $ renameShape hsc_env ispec (sh', pk')
            mergeShapes (decls, sh)
                        (L loc (shInclude incl_pk incl_sh), incl_sh)

shInclude :: PackageKey -> Shape -> ShUnitDecl
shInclude pk sh = ShInclude pk (fmap fst (sh_provides sh))
                               (fmap fst (sh_requires sh))
    ...
    = do let dflags = hsc_dflags hsc_env
         ...
         let renameRequires rns orig0 = do
                orig <- T.forM orig0 $ \(ms, avails) -> do
                             ms' <- mapM (renameHoleModule dflags hsubst) ms
                             avails' <- mapM (renameHoleAvailInfo hsc_env hsubst) avails
                             return (ms', avails')
                let go orig (L _ rn)
...
mergeModIface' merged_decls iface1 iface2 =
    ...
    let fixities = mergeFixities (mi_fixities iface1) (mi_fixities iface2)
        ...
        top_mod = TopModule (mi_semantic_module iface1) (mi_semantic_module iface1)
...
summariseDecl _dflags hsc_src mod lmodname Nothing
