...
shIncludeGraph pk lpkg = setPackageSh lpkg . setUnitIdSh pk $ do
    ...
    let go (gr, hsubst) (AcyclicSCC is0) = do -- TODO set loc
          ...
          rn_reqs  <- T.mapM (liftIO . renameHoleModule dflags hsubst) (is_requires is0)
...
shModGraph pk include_graph lpkg = setPackageSh lpkg . setUnitIdSh pk $ do
    ...
    let node_map = Map.fromList [ ((ms_mod_name n, ms_hsc_src n == HsigFile), n)
                                | n <- nodes ]
    req_nodes <- fmap catMaybes . forM insts $ \(mod_name, _) ->
        let has_local = Map.member (mod_name, True) node_map
        in if has_local
            then return Nothing
            else fmap Just $ summariseRequirement dflags include_graph mod_name
...
summariseRequirement :: DynFlags -> IncludeGraph -> ModuleName -> ShM ModSummary
summariseRequirement dflags include_graph mod_name = do
    hsc_env <- getTopEnv
    ComponentName unit_fs <- fmap shg_name getGblEnv
    location <- liftIO $ mkHomeModLocation2 dflags mod_name
                 (unpackFS unit_fs </> moduleNameSlashes mod_name) "hsig"
