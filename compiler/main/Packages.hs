...
-- | 'UniqFM' map from 'UnitKey'
type UnitKeyMap = UniqFM
...
type VisibilityMap =
    UnitKeyMap (Bool, [(ModuleName, ModuleName)], FastString)
...
data PackageState = PackageState {
  -- | A mapping of 'UnitKey' to 'PackageConfig'.  This list is adjusted
...
type InstalledPackageIndex = Map UnitKey PackageConfig
...
lookupPackage :: DynFlags -> UnitKey -> Maybe PackageConfig
...
-- not found
getPackageDetails :: DynFlags -> UnitKey -> PackageConfig
...
-- link in.
initPackages :: DynFlags -> IO (DynFlags, [UnitKey])
...
matchingKey str p = str == unitKeyString (packageConfigId p)

matching :: PackageArg -> PackageConfig -> Bool
matching (PackageArg str) = matchingStr str
matching (PackageIdArg str) = matchingId str
matching (UnitKeyArg str) = matchingKey str
...
packageFlagErr dflags flag reasons
  ...
  where err = text "cannot satisfy " <> pprFlag flag <>
        ...
        ppr_reason (p, reason) =
            pprReason (ppr (unitKey p) <+> text "is") reason
...
pprFlag flag = case flag of
  ...
  where ppr_arg arg = case arg of
                     ...
                     UnitKeyArg p -> text "-unit-key " <> text p
...
wired_in_pkgids = map unitKeyString wiredInUnitKeys

type WiredPackagesMap = Map UnitKey UnitKey
...
findWiredInPackages dflags pkgs vis_map = do
  ...
  let
        ...
        wired_in_ids = map unitKey wired_in_pkgs
        ...
        updateVisibilityMap vis_map = foldl' f vis_map wired_in_pkgs
          where f vm p = case lookupUFM vis_map (packageConfigId p) of
                            Nothing -> vm
                            Just r -> addToUFM vm (stringToUnitKey
...
data UnusablePackageReason
  = IgnoredWithFlag
  | MissingDependencies [UnitKey]

type UnusablePackages = Map UnitKey
                            (PackageConfig, UnusablePackageReason)
...
findBroken pkgs = go [] Map.empty pkgs
 where
   go avail keys not_avail =
     case partitionWith (depsAvailable keys) not_avail of
        ([], not_avail) ->
            Map.fromList [ (unitKey p, (p, MissingDependencies deps))
                         | (p,deps) <- not_avail ]
        (new_avail, not_avail) ->
            go (new_avail ++ avail) new_keys (map fst not_avail)
            where new_keys = Map.insertList
                                [ (unitKey p, p) | p <- new_avail ]
                                keys

   depsAvailable :: InstalledPackageIndex
                 -> PackageConfig
                 -> Either PackageConfig (PackageConfig, [UnitKey])
   depsAvailable keys pkg
        | null dangling = Left pkg
        | otherwise     = Right (pkg, dangling)
        where dangling = filter (not . (`Map.member` keys)) (depends pkg)
...
ignorePackages flags pkgs = Map.fromList (concatMap doit flags)
  ...
  doit (IgnorePackage str) =
     case partition (matchingStr str) pkgs of
         (ps, _) -> [ (unitKey p, (p, IgnoredWithFlag))
                    | p <- ps ]
...
-}
  ...
  -- it modifies the unit keys of wired in packages, but when we process
  ...
  --
  let preload1 = [ let key = unitKey p
...
mkModuleToPkgConfAll dflags pkg_db vis_map =
 ...
 where
  emptyMap = Map.empty
  sing key m _ = Map.singleton (mkModule key m)
     ...
     where origEntry = case lookupUFM esmap orig of
            ...
            Nothing -> throwGhcException (CmdLineError (showSDoc dflags
                        (text "package flag: could not find module name" <+>
                            ppr orig <+> text "in package" <+> ppr key)))

    es :: Bool -> [(ModuleName, Map Module ModuleOrigin)]
    es e = do
     -- TODO: signature support
     ExposedModule m exposedReexport _exposedSignature <- exposed_mods
     let (key', m', pkg', origin') =
          case exposedReexport of
           Nothing -> (key, m, pkg, fromExposedModules e)
           Just (OriginalModule key' m') ->
            let pkg' = pkg_lookup key'
            in (key', m', pkg', fromReexportedModules e pkg')
     return (m, sing key' m' pkg' origin')

    esmap :: UniqFM (Map Module ModuleOrigin)
    esmap = listToUFM (es False) -- parameter here doesn't matter, orig will
                                 -- be overwritten

    hiddens = [(m, sing key m pkg ModHidden) | m <- hidden_mods]

    key = packageConfigId pkg
...
getPackageIncludePath :: DynFlags -> [UnitKey] -> IO [String]
...
getPackageLibraryPath :: DynFlags -> [UnitKey] -> IO [String]
...
getPackageLinkOpts :: DynFlags -> [UnitKey] -> IO ([String], [String], [String])
...
getPackageExtraCcOpts :: DynFlags -> [UnitKey] -> IO [String]
getPackageExtraCcOpts dflags pkgs = do
  ps <- getPreloadPackagesAnd dflags pkgs
  return (concatMap ccOptions ps)

-- | Find all the package framework paths in these and the preload packages
getPackageFrameworkPath  :: DynFlags -> [UnitKey] -> IO [String]
getPackageFrameworkPath dflags pkgs = do
  ps <- getPreloadPackagesAnd dflags pkgs
  return (nub (filter notNull (concatMap frameworkDirs ps)))

-- | Find all the package frameworks in these and the preload packages
getPackageFrameworks  :: DynFlags -> [UnitKey] -> IO [String]
...
lookupModuleWithSuggestions dflags m mb_pn
  ...
  where
    ...
    mod_pkg = pkg_lookup . moduleUnitKey
...
-- 'PackageConfig's
getPreloadPackagesAnd :: DynFlags -> [UnitKey] -> IO [PackageConfig]
...
missingDependencyMsg :: Maybe UnitKey -> SDoc
missingDependencyMsg Nothing = Outputable.empty
missingDependencyMsg (Just parent)
  = space <> parens (ptext (sLit "dependency of") <+> ftext (unitKeyFS parent))

-- -----------------------------------------------------------------------------

unitKeyPackageIdString :: DynFlags -> UnitKey -> Maybe String
unitKeyPackageIdString dflags pkg_key
    | pkg_key == mainUnitKey = Just "main"
    | otherwise = fmap sourcePackageIdString (lookupPackage dflags pkg_key)

-- | Will the 'Name' come from a dynamically linked library?
isDllName :: DynFlags -> UnitKey -> Module -> Name -> Bool
...
pprPackagesSimple = pprPackagesWith pprIPI
    where pprIPI ipi = let i = unitKeyFS (unitKey ipi)
...
pprModuleMap dflags =
  vcat (map pprLine (Map.toList (moduleToPkgConfAll (pkgState dflags))))
    where
      ...
      pprEntry m (m',o)
        | m == moduleName m' = ppr (moduleUnitKey m') <+> parens (ppr o)
