...
flushFinderCaches hsc_env =
 ...
 where
        ...
        is_ext mod _ | moduleUnitKey mod /= this_pkg = True
                     | otherwise = False
...
findExactModule hsc_env mod =
    ...
    in if moduleUnitKey mod == thisPackage dflags
...
findExposedPackageModule hsc_env mod_name mb_pkg
  = case lookupModuleWithSuggestions (hsc_dflags hsc_env) mod_name mb_pkg of
     LookupFound m pkg_conf ->
       ASSERT( moduleUnitKey m == packageConfigId pkg_conf )
       findPackageModule_ hsc_env m (importDirs pkg_conf)
     LookupMultiple rs ->
       return (FoundMultiple rs)
     LookupHidden pkg_hiddens mod_hiddens ->
       return (NotFound{ fr_paths = [], fr_pkg = Nothing
                       , fr_pkgs_hidden = map (moduleUnitKey.fst) pkg_hiddens
                       , fr_mods_hidden = map (moduleUnitKey.fst) mod_hiddens
                       , fr_suggestions = [] })
...
--
--  3. When we look up an exact 'Module', if the unit key associated with
...
findPackageModule hsc_env mod = do
  let
        dflags = hsc_dflags hsc_env
        pkg_id = moduleUnitKey mod
  --
  case lookupPackage dflags pkg_id of
     Nothing -> return (NoPackage pkg_id)
     Just pkg_conf ->
        ASSERT( moduleUnitKey mod == packageConfigId pkg_conf )
...
-- the 'PackageConfig' must be consistent with the unit key in the 'Module'.
...
findIndefinitePackageModule hsc_env mod = do
    let dflags = hsc_dflags hsc_env
        pk = moduleUnitKey mod
    uid <- unitKeyUnitId dflags pk
    case lookupIndefiniteUnit dflags uid of
        IndefNotFound -> return (NoPackage pk)
        IndefFoundIndefinite conf -> findPackageModule_ hsc_env mod (indefImportDirs conf)
        IndefFoundDefinite conf -> findPackageModule_ hsc_env mod (importDirs conf)
...
cantFindErr _ multiple_found _ mod_name (FoundMultiple mods)
  ...
  where
    ...
    pprOrigin m (ModOrigin e res _ f) = sep $ punctuate comma (
      if e == Just True
          then [ptext (sLit "package") <+> ppr (moduleUnitKey m)]
          else [] ++
...
cantFindErr cannot_find _ dflags mod_name find_result
  ...
  where
    more_info
      = case find_result of
            NoPackage pkg
                -> ptext (sLit "no unit key matching") <+> quotes (ppr pkg) <+>
    ...
    pkg_hidden :: UnitKey -> SDoc
    pkg_hidden pkgid =
        ptext (sLit "It is a member of the hidden package")
        <+> quotes (ppr pkgid)
        --FIXME: we don't really want to show the unit key here we should
    ...
    looks_like_srcpkgid :: UnitKey -> SDoc
    looks_like_srcpkgid pk
     -- Unsafely coerce a unit key FastString into a source package ID
     -- FastString and see if it means anything.
     | (pkg:pkgs) <- searchPackageId dflags (SourcePackageId (unitKeyFS pk))
     = parens (text "This unit key looks like the source package ID;" $$
       text "the real unit key is" <+> quotes (ftext (unitKeyFS (unitKey pkg))) $$
    ...
    pp_sugg (SuggestVisible m mod o) = ppr m <+> provenance o
      where provenance ModHidden = Outputable.empty
            provenance (ModOrigin{ fromOrigPackage = e,
                                   ...
                                   fromPackageFlag = f })
              | Just True <- e
                 = parens (ptext (sLit "from") <+> ppr (moduleUnitKey mod))
              | f && moduleName mod == m
                 = parens (ptext (sLit "from") <+> ppr (moduleUnitKey mod))
              | (pkg:_) <- res
    ...
    pp_sugg (SuggestHidden m mod o) = ppr m <+> provenance o
      where provenance ModHidden =  Outputable.empty
            provenance (ModOrigin{ fromOrigPackage = e,
              ...
              | Just False <- e
                 = parens (ptext (sLit "needs flag -unit-key")
                    <+> ppr (moduleUnitKey mod))
              | (pkg:_) <- rhs
                 = parens (ptext (sLit "needs flag -unit-key")
