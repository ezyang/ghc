...
type HomePackageTable  = ModuleNameEnv HomeModInfo
        ...
        -- "home" unit key cached here for convenience
...
type ExternalShapeTable = Map UnitName (Shape, UnitKey)
...
data FindResult
  ...
  | NotFound
      ...
      , fr_pkg         :: Maybe UnitKey  -- Just p => module is in this package's
                                           --           manifest, but couldn't find
                                           --           the .hi file

      , fr_mods_hidden :: [UnitKey]      -- Module is in these packages,
                                           --   but the *module* is hidden

      , fr_pkgs_hidden :: [UnitKey]      -- Module is in these packages,
...
data CgGuts
  = CgGuts {
        ...
        cg_dep_pkgs  :: ![UnitKey],    -- ^ Dependent packages, used to
...
common package 'interactive' (see Module.interactiveUnitKey, and
...
setInteractivePackage hsc_env
   = hsc_env { hsc_dflags = (hsc_dflags hsc_env) { thisPackage = interactiveUnitKey } }
...
Note [Printing unit keys]
~~~~~~~~~~~~~~~~~~~~~~~~~~~~
In the old days, original names were tied to PackageIds, which directly
corresponded to the entities that users wrote in Cabal files, and were perfectly
suitable for printing when we need to disambiguate packages.  However, with
UnitKey, the situation can be different: if the key is instantiated with
...
mkPrintUnqualified dflags env = QueryQualify qual_name
  ...
  qual_name mod occ
        | [] <- unqual_gres
        , moduleUnitKey mod `elem` [primUnitKey, baseUnitKey, thUnitKey]
...
mkQualModule dflags mod
     | moduleUnitKey mod == thisPackage dflags = False

     | [(_, pkgconfig)] <- lookup,
       packageConfigId pkgconfig == moduleUnitKey mod
...
-- with a unit key if the package ID would be ambiguous.
mkQualPackage :: DynFlags -> QueryQualifyPackage
mkQualPackage dflags pkg_key
     | pkg_key == mainUnitKey || pkg_key == interactiveUnitKey
