...
hscGenHardCode hsc_env cgguts mod_summary output_filename = do
        ...
        let (cmms, foreign_stubs) =
                prepareStaticClosures (Right this_mod) cmms0 foreign_stubs1
...
hscCompileCmmFile hsc_env filename output_filename = runHsc hsc_env $ do
    ...
    liftIO $ do
        ...
        (_, (_, stub_c_exists)) <- codeOutput dflags no_mod output_filename
                                              no_loc foreign_stubs [] rawCmms
...
prepareStaticClosures lbl_or_mod cmms0 foreign_stubs0 =
    let cmms = deferStaticClosures lbl_or_mod cmms0 [] []
        ...
        static_closure_inds_init = vcat
            [ text "static void static_closure_inds_init_" <> tag
                <> text "(void) __attribute__((constructor));"
            , text "extern StgClosure *"
                <> ppr (mkStaticClosureIndsLabel lbl_or_mod True) <> text ";"
            , text "extern StgClosure *"
                <> ppr (mkStaticClosureIndsLabel lbl_or_mod False) <> text ";"
            , text "static StaticClosureInds sic_table_" <> tag
                <> text " = { &"
                <> ppr (mkStaticClosureIndsLabel lbl_or_mod True)
                <> text ", &"
                <> ppr (mkStaticClosureIndsLabel lbl_or_mod False)
                <> text ", NULL};"
            , text "static void static_closure_inds_init_" <> tag
                <> text "(void)"
            , braces (text "REGISTER_STATIC_CLOSURE_INDS(&sic_table_" <> tag
                        <> text ");")
            ]
