...
checkSafeImports dflags tcg_env
  ...
  where
    ...
    pkgReqs  = imp_trust_pkgs impInfo  -- [UnitKey]
...
hscGetSafe :: HscEnv -> Module -> SrcSpan -> IO (Bool, [UnitKey])
...
hscCheckSafe' :: DynFlags -> Module -> SrcSpan -> Hsc (Maybe UnitKey, [UnitKey])
hscCheckSafe' dflags m l = do
    (tw, pkgs) <- isModSafe m l
    case tw of
        False              -> return (Nothing, pkgs)
        True | isHomePkg m -> return (Nothing, pkgs)
             | otherwise   -> return (Just $ moduleUnitKey m, pkgs)
  where
    isModSafe :: Module -> SrcSpan -> Hsc (Bool, [UnitKey])
    isModSafe m l = do
        iface <- lookup' m
        case iface of
                ...
                where
                    pkgTrustErr = unitBag $ mkErrMsg dflags l (pkgQual dflags) $
                        sep [ ppr (moduleName m)
                            ...
                            , text "The package (" <> ppr (moduleUnitKey m)
    ...
    packageTrusted _ _ m
        | isHomePkg m = True
        | otherwise   = trusted $ getPackageDetails dflags (moduleUnitKey m)
    ...
    isHomePkg m
        | thisPackage dflags == moduleUnitKey m = True
        | otherwise                               = False

-- | Check the list of packages are trusted.
checkPkgTrust :: DynFlags -> [UnitKey] -> Hsc ()
...
hscParsedStmt hsc_env stmt = runInteractiveHsc hsc_env $ do
  ...
  -- It's important NOT to have package 'interactive' as thisUnitKey
