  ...
  where
    ...
    initial_graph = case mb_root_mod of
        ...
        Just root_mod ->
            ...
            -- the specified node.
            let root | Just node <- lookup_node NotBoot root_mod, graph `hasVertexG` node = node
...
moduleGraphNodes drop_hs_boot_nodes summaries = (graphFromEdgedVertices nodes, lookup_node)
  where
    ...
    nodes = [ (s, key, out_keys)
            ...
            , not (isBootSummary s && hasImpl (ms_mod_name s) && drop_hs_boot_nodes)
            , let out_keys = out_edge_keys IsBoot  (map unLoc (ms_home_srcimps s)) ++
                             out_edge_keys NotBoot (map unLoc (ms_home_imps s)) ++
                             (if fst (ms_merge_imps s) then out_edge_keys IsBoot [moduleName (ms_mod s)] else []) ++
   ...
   = do
     ...
     where
        ...
        calcDeps summ
          ...
          , moduleUnitKey m == thisPackage (hsc_dflags hsc_env)
...
msDeps s =
    concat [ [(m,IsBoot), (m,NotBoot)] | m <- ms_home_srcimps s ]
        ...
        ++ if fst (ms_merge_imps s) then [ (noLoc (moduleName (ms_mod s)), IsBoot) ] else []
...
summariseModule hsc_env old_summary_map is_boot (L loc wanted_mod)
  ...
  = do mod_summary0 <- makeMergeRequirementSummary hsc_env obj_allowed wanted_mod
    ...
    find_it = do
        ...
        case found of
             Found location mod
                ...
                | otherwise ->
                        -- Drop external-pkg
                        ASSERT(moduleUnitKey mod /= thisPackage dflags)
                        return Nothing
