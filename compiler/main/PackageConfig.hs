...
packageConfigId p =
    if isEmptyUniqDSet (unionManyUniqDSets (map (moduleFreeHoles.snd) (instantiatedWith p)))
        then DefiniteUnitId (DefUnitId (unitId p))
        else newUnitId (installedUnitIdComponentId (unitId p)) (instantiatedWith p)
