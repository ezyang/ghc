...
linkingNeeded :: DynFlags -> Bool -> [Linkable] -> [UnitKey] -> IO Bool
...
checkLinkInfo :: DynFlags -> [UnitKey] -> FilePath -> IO Bool
        ...
        liftIO $ SysTools.runCc dflags (
                ...
                -- the base package or not.
                       ++ (if platformOS platform == OSMinGW32 &&
                              thisPackage dflags == baseUnitKey
...
mkExtraObj dflags extn xs
 = do cFile <- newTempName dflags extn
      ...
      writeFile cFile xs
      let rtsDetails = getPackageDetails dflags rtsUnitKey
...
mkNoteObjsToLinkIntoBinary :: DynFlags -> [UnitKey] -> IO [FilePath]
...
getLinkInfo :: DynFlags -> [UnitKey] -> IO String
...
getHCFilePackages :: FilePath -> IO [UnitKey]
getHCFilePackages filename =
  Exception.bracket (openFile filename ReadMode) hClose $ \h -> do
    l <- hGetLine h
    case l of
      '/':'*':' ':'G':'H':'C':'_':'P':'A':'C':'K':'A':'G':'E':'S':rest ->
          return (map stringToUnitKey (words rest))
      _other ->
          return []
...
linkBinary :: DynFlags -> [FilePath] -> [UnitKey] -> IO ()
linkBinary = linkBinary' False

linkBinary' :: Bool -> DynFlags -> [FilePath] -> [UnitKey] -> IO ()
...
linkDynLibCheck :: DynFlags -> [String] -> [UnitKey] -> IO ()
...
linkStaticLibCheck :: DynFlags -> [String] -> [UnitKey] -> IO ()
...
getGhcVersionPathName dflags = do
  dirs <- getPackageIncludePath dflags [rtsUnitKey]
