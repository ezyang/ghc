...
tcRnSignature hsc_src
 = do { tcg_env <- getGblEnv
      ; let inner_mod = topModSemantic (tcg_top_mod tcg_env)
            outer_mod = topModIdentity (tcg_top_mod tcg_env)
      ; case hsc_src of
            -- If you're doing old-style mutual recursion, the inner_mod
            -- and outer_mod will coincide.  Don't try to load inner_mod
            -- in that case, it won't work!
            -- (This won't work for cross package mutual recursion, you'd
            -- need to adjust this to fill out tcg_impl_rdr_env from
            -- the shape.  The test should be to check the SHAPE before
            -- trying to load the interface which might be bogus)
            HsBootFile | inner_mod /= outer_mod
...
pprTcGblEnv (TcGblEnv { tcg_type_env  = type_env,
  ...
  = vcat [ ppr_types insts type_env
         ...
         , ptext (sLit "Dependent packages:") <+>
                ppr (sortBy stableUnitKeyCmp $ imp_dep_pkgs imports)]
