...
instance TH.Quasi (IOEnv (Env TcGblEnv TcLclEnv)) where
  ...
  qLocation = do { m <- getModule
                 ...
                 ; return (TH.Loc { TH.loc_filename = unpackFS (srcSpanFile r)
                                  ...
                                  , TH.loc_package  = unitKeyString (moduleUnitKey m)
...
reifyName thing
  ...
  where
    ...
    pkg_str = unitKeyString (moduleUnitKey mod)
...
lookupThAnnLookup (TH.AnnLookupModule (TH.Module pn mn))
  = return $ ModuleTarget $
    mkModule (stringToUnitKey $ TH.pkgString pn) (mkModuleName $ TH.modString mn)
...
modToTHMod m = TH.Module (TH.PkgName $ unitKeyString  $ moduleUnitKey m)
                         (TH.ModName $ moduleNameString $ moduleName m)

reifyModule :: TH.Module -> TcM TH.ModuleInfo
reifyModule (TH.Module (TH.PkgName pkgString) (TH.ModName mString)) = do
  this_mod <- getModule
  let reifMod = mkModule (stringToUnitKey pkgString) (mkModuleName mString)
    ...
    where
      ...
      reifyFromIface reifMod = do
        ...
        let usages = [modToTHMod m | usage <- mi_usages iface,
                                     Just m <- [usageToModule (moduleUnitKey reifMod) usage] ]
        return $ TH.ModuleInfo usages

      usageToModule :: UnitKey -> Usage -> Maybe Module
