...
emptyPLS _ = PersistentLinkerState {
  ...
  where init_pkgs = [rtsUnitKey]


extendLoadedPkgs :: [UnitKey] -> IO ()
...
getLinkDeps :: HscEnv -> HomePackageTable
            ...
            -> IO ([Linkable], [UnitKey])     -- ... then link these first
 ...
 = do {
  ...
  where
    ...
    follow_deps :: [Module]             -- modules to follow
                ...
                -> UniqSet UnitKey          -- accum. package dependencies
                -> IO ([ModuleName], [UnitKey]) -- result
...
-- packages.
--
linkPackages :: DynFlags -> [UnitKey] -> IO ()
...
linkPackages' :: DynFlags -> [UnitKey] -> PersistentLinkerState
             -> IO PersistentLinkerState
linkPackages' dflags new_pks pls = do
    pkgs' <- link (pkgs_loaded pls) new_pks
    return $! pls { pkgs_loaded = pkgs' }
  where
     link :: [UnitKey] -> [UnitKey] -> IO [UnitKey]
     link pkgs new_pkgs =
     ...
     link_one pkgs new_pkg
        ...
        | otherwise
        = throwGhcExceptionIO (CmdLineError ("unknown package: " ++ unitKeyString new_pkg))
