...
-- TODO: need to be a bit more careful distinguishing hashed uids and
-- unhashed uids
...
changeDBDir verbosity cmds db = do
 ...
 where
  do_cmd (RemovePackage p) = do
    let file = location db </> unGhcUnitId (hashInstalledUnitId p) <.> "conf"
...
-- Don't use a string directly, because we have to apply a conversion step here.
newtype GhcUnitId = GhcUnitId { unGhcUnitId :: String }
    deriving (GhcPkg.BinaryStringRep, Ord, Eq, Show)

-- Garden variety specialized writer monad
newtype HashMonad a = HashMonad { runHashMonad :: (Map GhcUnitId IndefiniteUnitId, a) }

instance Functor HashMonad where
    fmap = liftM

instance Applicative HashMonad where
    pure a = HashMonad (Map.empty, a)
    (<*>) = ap

instance Monad HashMonad where
    return = pure
    HashMonad (w, a) >>= f
        = HashMonad $ let HashMonad (w', b) = f a
                      in (Map.union w w', b)

type IndefiniteUnitId = GhcPkg.IndefiniteUnitId GhcUnitId ComponentId ModuleName

addHash :: IndefiniteUnitId -> GhcUnitId -> HashMonad ()
addHash uid uid' = HashMonad (Map.singleton uid' uid, ())

hashUnitId :: UnitId -> HashMonad GhcUnitId
hashUnitId orig_uid@(UnitId cid hmap) = do
    hmap' <- mapM (\(k, Module uid m) -> do
                        uid' <- hashUnitId uid
                        return (k, GhcPkg.Module uid' m)) hmap
    let uid' = GhcPkg.mkUnitId cid hmap'
    addHash (GhcPkg.IndefiniteUnitId cid hmap') uid'
    return uid'

hashInstalledUnitId :: InstalledPackageInfo -> GhcUnitId
hashInstalledUnitId = snd . runHashMonad . hashUnitId . installedUnitId
...
convertPackageInfoToCacheFormat pkg =
    GhcPkg.InstalledPackageInfo {
       ...
       GhcPkg.unitId             = pkgUnitId,
       GhcPkg.componentName      = display (packageName pkg), -- TODO
       GhcPkg.depends            = pkgDepends,
       ...
       GhcPkg.exposedModules     = pkgExposedModules,
       GhcPkg.hiddenModules      = hiddenModules pkg,
       GhcPkg.instantiatedDepends= pkgInstantiatedDepends,
       GhcPkg.indefinite         = indefinite pkg,
       GhcPkg.unitIdMap          = map (\(a,b) -> (a,b)) (Map.toList uid_map),
       GhcPkg.exposed            = exposed pkg,
       GhcPkg.trusted            = trusted pkg
    }
  where (uid_map, (pkgUnitId, pkgDepends, pkgInstantiatedWith, pkgInstantiatedDepends, pkgExposedModules)) = runHashMonad $ do
            pkgUnitId <- hashUnitId (installedUnitId pkg)
            pkgDepends <- mapM hashUnitId (depends pkg)
            pkgInstantiatedDepends <- mapM hashUnitId (instantiatedDepends pkg)
            pkgExposedModules <- mapM convertExposed (exposedModules pkg)
            return (pkgUnitId, pkgDepends, pkgInstantiatedWith, pkgInstantiatedDepends, pkgExposedModules)
        convertExposed (ExposedModule n reexport) = do
            mod <- case reexport of
                    Nothing -> do uid <- hashUnitId (installedUnitId pkg)
                                  return (GhcPkg.Module uid n)
                    Just mod -> convertOriginal mod
            return (n, mod)
        convertOriginal (Module uid m) = do
            uid' <- hashUnitId uid
            return (GhcPkg.Module uid' m)
        convertInst (m, o) = do
            orig' <- convertOriginal o
            return (m, orig')
...
instance GhcPkg.BinaryStringRep ComponentId where
  fromStringRep = ComponentId . fromUTF8 . BS.unpack
  toStringRep   = BS.pack . toUTF8 . display
      ...
      show_normal PackageDB{ location = db_name, packages = pkg_confs } =
          do hPutStrLn stdout (db_name ++ ":")
           ...
           where
                 ...
                 pp_pkgs = map pp_pkg . sortBy (comparing installedUnitId) $ pkg_confs
                 pp_pkg p
                   | installedUnitId p `elem` broken = printf "{%s}" doc
                   | exposed p = doc
                   | otherwise = printf "(%s)" doc
                   where doc | verbosity >= Verbose = printf "%s (%s)" pkg (unGhcUnitId uid)
                             | otherwise            = pkg
                          where
                          uid = hashInstalledUnitId p
          ...
          where
                   pp_pkg p
                     ...
                     where doc | verbosity >= Verbose
                               = termText (printf "%s (%s)" pkg (unGhcUnitId uid))
                               | otherwise
                               = termText pkg
                            where
                            uid = hashInstalledUnitId p
...
showPackageDot verbosity myflags = do
  ...
  mapM_ putStrLn [ quote from ++ " -> " ++ quote to
                 | p <- all_pkgs,
                   ...
                   UnitId cid _ <- depends p,
                   Just dep <- [PackageIndex.lookupComponentId ipix cid],
...
checkModule field_name db_stack pkg
  ...
  let mpkg = if definingPkgId == installedUnitId pkg
              ...
              else PackageIndex.lookupComponentId ipix definingComponentId
      definingComponentId = unitIdComponentId definingPkgId
  in case mpkg of
      Nothing
      ...
      Just definingPkg
        | not (isIndirectDependency definingComponentId)
